...
class ContractCreateFlow {
  ...
  async executeWithSigner(signer) {
    ...
    let response = await new _FileCreateTransaction.default().setKeys(key != null ? [key] : []).setContents(this._bytecode.subarray(0, Math.min(this._bytecode.length, 2048))).executeWithSigner(signer);
    ...
    if (this._bytecode.length > 2048) {
      await new _FileAppendTransaction.default().setFileId(fileId).setContents(this._bytecode.subarray(2048)).executeWithSigner(signer);
    }

    response = await this._contractCreate.setBytecodeFileId(fileId).executeWithSigner(signer);
    await response.getReceiptWithSigner(signer);

    if (key != null) {
      await (await new _FileDeleteTransaction.default().setFileId(fileId).executeWithSigner(signer)).getReceiptWithSigner(signer);
    }
