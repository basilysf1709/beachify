...
async function createFile(callData, client) {
  const fileId =
  /** @type {FileId} */
  (await (await new _FileCreateTransaction.default().setContents(callData.subarray(0, Math.min(callData.length, 4096))).execute(client)).getReceipt(client)).fileId;

  if (callData.length > 4096) {
    await (await new _FileAppendTransaction.default().setFileId(fileId).setContents(callData.subarray(4096)).execute(client)).getReceipt(client);
  }
