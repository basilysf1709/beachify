...
var _sync = _interopRequireDefault(require("./sync.cjs"));
...
class Executable {
  ...
  async execute(client, requestTimeout) {
    ...
    await _sync.default; // If the request timeout is set on the request we'll prioritize that instead
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (!node.isHealthy()) {
        _jsLogger.default.debug(`[${logId}] node is not healthy, skipping waiting ${node.getRemainingTime()}`);
